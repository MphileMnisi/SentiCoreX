...
import { analyzeSentiment, performAdvancedAnalysis, getMoodEnhancers } from './services/geminiService';
import {
  ActiveTab, ApiStatus, Sentiment, SentimentAnalysisResult, AdvancedAnalysisResult, Entity, MoodEnhancerResult
} from './types';
import {
  BatchIcon, ChartLineIcon, CsvIcon, HistoryIcon, JsonIcon, MicIcon, MoonIcon, MusicIcon, PdfIcon,
  QuoteIcon, SearchIcon, SparklesIcon, SunIcon, UploadIcon
...
const Header: React.FC<{ theme: Theme; toggleTheme: () => void }> = ({ theme, toggleTheme }) => (
    <header className="relative text-center mb-8 p-8 bg-gradient-to-r from-purple-600 via-blue-600 to-cyan-500 text-white rounded-2xl shadow-2xl overflow-hidden animate-gradient">
        ...
        <h1 className="text-4xl md:text-5xl font-bold mb-3 flex items-center justify-center gap-3 animate-fade-in">
            <ChartLineIcon className="w-10 h-10" /> AI Sentiment Analysis Pro
        </h1>
        <p className="text-xl opacity-90">Advanced emotional intelligence powered by AI</p>
    </header>
);
...
const InputSection: React.FC<{ onAnalyze: (text: string | string[]) => void; isLoading: boolean }> = ({ onAnalyze, isLoading }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-darker/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-gray-light dark:border-gray-dark animate-slide-up">
            <div className="flex border-b border-gray-light dark:border-gray-dark mb-6">
                {tabs.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-3 font-semibold text-sm transition-all ${activeTab === tab.id ? 'border-b-2 border-purple-600 text-purple-600' : 'text-gray-500 hover:text-dark dark:hover:text-light'}`}>
                        {tab.icon} {tab.label}
                    </button>
                ))}
            </div>
            ...
            {activeTab === ActiveTab.BatchProcessing && <div>
                ...
                <button onClick={handleAnalyzeClick} disabled={isLoading || !batchInput.trim()} className="mt-4 w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white font-bold py-3 rounded-xl hover:scale-105 transition-transform disabled:opacity-50 flex items-center justify-center gap-2">{isLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <SearchIcon className="w-5 h-5" />} Analyze Batch</button>
            </div>}
...
const VisualizationSection: React.FC<{ results: SentimentAnalysisResult[], advancedResult: AdvancedAnalysisResult | null }> = ({ results, advancedResult }) => {
    const chartData = useMemo(() => {
        ...
        if (results.length > 1) {
            ...
            // FIX: Cast `value` to number before arithmetic operation as Object.entries returns `unknown` for values.
            breakdown = Object.entries(results[0].scores).map(([name, value]) => ({ name: name as Sentiment, value: Math.round((value as number) * 100) }));
        }
